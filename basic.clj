...
(defn spy [x y] (println x " es: " y) y) ; Para debugging
        ...
        (fn [[rpn pila] token]
          (let [op-mas? #(and (some? (precedencia %)) (>= (precedencia %) (precedencia token)))
            ...
            (cond
              ...
              (and (= (str token) ",") (operador? (first pila))) [(conj rpn (first pila) token) (drop 1 pila)]
  ...
  (if (or (contains? (set sentencia) nil) (and (palabra-reservada? (first sentencia)) (= (second sentencia) '=)))
      ...
      (case (first sentencia)
        ...
        NEXT (if (<= (count (next sentencia)) 1)
                  ...
                  (do (dar-error 16 (amb 1)) [nil amb]))  ; Syntax error
        LIST (do (println (amb 0)) [:sin-errores amb])
        LET [:sin-errores (ejecutar-asignacion (drop 1 sentencia) amb)]
        END [:omitir-restante ['() [:ejecucion-inmediata 0] [] [] [] 0 {}]]
    ...
    (if (or (nil? operando1) (nil? operando2))
        ...
        (case operador
          * (* operando1 operando2)
          ...
          MID$ (if (< operando2 1) ;Iba operando2
          ...
          < (if (< operando1 operando2) 1 0)
          > (if (> operando1 operando2) 1 0)
          LEFT$ (apply str (drop-last (- (count operando1) operando2) operando1))
          (<> ><) (if (and (string? operando1) (string? operando2))
...
(defn palabra-reservada? [x]
  (contains? #{"LOAD" "SAVE" "INPUT" "PRINT" "?" "DATA" "READ" "REM" "RESTORE" "CLEAR" "LET" 
               "=" "LIST" "NEW" "RUN" "END" "FOR" "TO" "NEXT" "STEP" "GOSUB" "RETURN" "GOTO" 
               "IF" "THEN" "ON" "ENV" "EXIT" "ATN" "INT" "SIN" "LEN" "MID$" "MID3$" "ASC" "CHR$" 
               "STR$" "+" "-" "*" "/" "^" "<>" "<" "<=" ">" ">=" "AND" "OR" "NOT" "-u"
               "LEFT$"} (str x))
)
...
(defn anular-invalidos [sentencia]
  (map (fn [sym] (if (contains? #{"&" "!"} (str sym)) nil sym)) sentencia)
)
...
;No la uso anónimamente, para que sea más claro
(defn mapa-a-lista [mapa] 
  (map 
    (fn [vec] (as-> vec v
      (reverse (into '() v))
      (concat (list (first v)) (second v))
    ))
  (reverse (into '() mapa)))
)

;No la uso anónimamente, para que sea más claro
(defn lista-a-mapa [lista]
  (into (sorted-map) (for [linea lista] [(first linea) (rest linea)]))
)

(defn cargar-linea [linea amb]
  (let [mapa (lista-a-mapa (first amb))]
    (cond
      (= (count linea) 1) (assoc amb 0 (mapa-a-lista (dissoc mapa (first linea))))
      (> (count linea) 1) (assoc amb 0 (mapa-a-lista (assoc mapa (first linea) (rest linea))))
      :else amb 
    )
  )
)
...
;No la uso anónimamente, para que sea más claro
(defn exp-aux [sent] 
  (let [next-aux (fn [sent]
                    (as-> sent s
                      (remove (fn [lit] (= (str lit) ",")) s)
                      (for [i (range 1 (count s))] (conj (list (nth s i)) 'NEXT))
                    )
                  )
        ]
    (if (and (= (first sent) 'NEXT)  (> (count sent) 2)) (next-aux sent) (list sent))
  )  
)

(defn expandir-nexts [n]
  (apply concat (map exp-aux n))
)
...
(defn dar-error [cod prog-ptrs] 
  (let [in (if (number? (first prog-ptrs)) (str " IN " (first prog-ptrs)) "")]
    (if (number? cod) (print (str (buscar-mensaje cod) in)) (print (str cod in)))
  )
)
...
;Esta función también se utiliza mas adelante en otras
(defn es-letra? [caract]
  (cond
    (contains? (into #{} (map char (range (int \a) (inc (int \z))))) caract) true
    (contains? (into #{} (map char (range (int \A) (inc (int \Z))))) caract) true
    :else false
  )
)
...
(defn variable-float? [x]
  (let [input (str x)]
    (if (= 1 (count input)) (es-letra? (first input)) false)   
  )
)
...
(defn variable-integer? [x]
  (let [input (str x)]
    (if (= 2 (count input)) (and (es-letra? (first input)) (= \% (second input))) false)   
  )
)
...
(defn contar-sentencias [nro-linea amb]
  (count (expandir-nexts (get (lista-a-mapa (first amb)) nro-linea)))
)
...
(defn buscar-lineas-restantes
  ...
  ([act prg] (let [idx (first act) mapa (lista-a-mapa prg)]
    (cond
      (not (number? idx)) nil
      (not (contains? mapa idx)) nil
      :else (as-> mapa m
              (into (sorted-map) (filter (fn [[k v]] (>= k idx)) m))
              (let [sents (expandir-nexts (get m idx))]
                (assoc m idx (drop (- (count sents) (second act)) sents))
              )
              (mapa-a-lista m)
      )
    ) 
...
(defn continuar-linea [amb]
  (let [sub-stack (first (amb 2))
        actualizar-amb (fn [amb] 
                          (as-> amb a
                            (assoc a 2 [])
                            (let [sec-substk (- (second sub-stack) 1)]
                              (assoc a 1 (if (< sec-substk 0) [(first sub-stack) 0] [(first sub-stack) sec-substk]))
                            )
                          )
                        )
      ]
    (if (nil? sub-stack)
      ...
      [':omitir-restante (actualizar-amb amb)]
    )
  )
)
...
(defn ext-aux [line]
  (let [data-coma (fn [sent]  
                      (as-> sent s
                        (drop 1 s)
                        (remove (fn [lit] (= (str lit) ",")) s)
                        (map (fn [sym] (if (not (number? sym)) (str sym) sym)) s)    
                      ) 
                  ) 
        sents (drop 1 line)]
    (if (not-any? (fn [sent] (= 'REM (first sent))) sents) 
      (as-> sents ss
        (map (fn [sent] (if (= 'DATA (first sent)) (data-coma sent) nil)) ss)
        (filter (fn [v] (not (nil? v))) ss)
      ) 
      '()
    )
  )
...
(defn ejecutar-asignacion [sentencia amb]
  (let [vmen (amb 6)]
    (assoc amb 6 (assoc vmen (first sentencia) (calcular-expresion (drop 2 sentencia) amb)))
  )
)
...
;X$ string, X float, X% int, en caso de no existir en var-mem se reemplaza por "", 0, 0 respectivamente

(defn pre-aux [simbolo]
  (let [s (str simbolo)]
    (cond
      (= s ".") 0
      (= (count s) 2) (case (second s) \$ "" \% 0 simbolo)
      (and (= (count s) 1) (symbol? simbolo) (es-letra? (first s))) 0
      :else simbolo
    )
  )
)
...
(defn preprocesar-expresion [expr amb]
  (map (fn [simbolo] (get (nth amb 6) simbolo (pre-aux simbolo))) expr)
)
...
(defn precedencia [token]
  (if (and (palabra-reservada? token) (not (operador? token)))
    9
    (case (str token)
      "OR" 1
      "AND" 2
...
(defn aridad [token]
  (cond
    (or (number? token) (= "*" token)) 0 ; El "*" es para STARS.BAS
    (operador? token) (if (= '-u token) 1 2)
    (palabra-reservada? token)
      (case token 
        THEN 0 
        (INT ATN SIN PRINT -u LEN STR$ CHR$ ASC) 1 
        (MID$ LEFT$) 2 
        MID3$ 3
      )
    :else 0
...
(defn eliminar-cero-decimal [n]
  (if (float? n)
    (as-> n num
      (str num)
      (reverse num)
      (drop-while (fn [c] (= c \0)) num)
      (reverse num)
      (if (= \. (last num)) (drop-last num) num)
      (apply str num)
      (read-string num)
    )
    n
...
(defn eliminar-cero-entero [n] 
  (cond
    (nil? n) n
    (number? n) (if (and (< n 1) (> n -1) (not= n 0)) (clojure.string/replace (str n) "0" "") (str n))
    :else (str n)
  )
)
